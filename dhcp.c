...
int get_hardware_address(int sock, char *interface_name) {
	...
	if (mymac) {
		...
		if ( ret < 0) {
			printf("Error: Could not set hardware address of interface '%s', ret=%d\n",
					interface_name, ret);
			// perror("Error");
			// exit(STATE_UNKNOWN);
		}
...
/* sends a DHCPREQUEST broadcast message in an attempt to find DHCP servers */
...
int get_dhcp_offer(int sock) {
	...
	for (responses = 0, valid_responses = 0;;) {
		...
		if (result == ERROR) {
			if (verbose)
				printf(
						"DHCPOFFER hardware address did not match our own - ignoring packet\n");
...
int send_dhcp_packet(void *buffer, int buffer_size, int sock,
	...
	result = sendto(sock, (char *) buffer, buffer_size, 0,
...
int add_dhcp_offer(struct in_addr source, dhcp_packet *offer_packet) {
	...
	if (verbose) {
		printf("Added offer from server @ %s",
...
int free_requested_server_list(void) {
	...
	for (this_server = requested_server_list; this_server != NULL; this_server =
			next_server) {
...
int get_results(void) {
	...
	if (requested_servers > 0) {
		...
		for (temp_server = requested_server_list; temp_server != NULL;
			...
			for (temp_offer = dhcp_offer_list; temp_offer != NULL; temp_offer =
					temp_offer->next) {

				/* get max lease time we were offered */
				if (temp_offer->lease_time
						> max_lease_time|| temp_offer->lease_time==DHCP_INFINITE_TIME)
				...
				if (!memcmp(&temp_offer->server_address,
					...
					if (verbose) {
						printf("DHCP Server Match: Offerer=%s",
	...
	if (request_specific_address == TRUE)
		printf(", requested address (%s) was %soffered",
...
#ifdef HAVE_GETOPT_H
	int option_index = 0;
	static struct option long_options[] = { { "serverip", required_argument, 0,
			's' }, { "requestedip", required_argument, 0, 'r' }, { "timeout",
			required_argument, 0, 't' }, { "interface", required_argument, 0,
			'i' }, { "mac", required_argument, 0, 'm' }, { "verbose",
			no_argument, 0, 'v' }, { "version", no_argument, 0, 'V' }, { "help",
			no_argument, 0, 'h' }, { 0, 0, 0, 0 } };
#endif

	while (1) {
#ifdef HAVE_GETOPT_H
		c = getopt_long(argc, argv, "+hVvt:s:r:t:i:m:", long_options,
				&option_index);
#else
		...
		{
			int ret;
			ret = sscanf(optarg, "%x:%x:%x:%x:%x:%x", my_client_mac + 0,
